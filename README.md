<h1 align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=28&duration=3000&pause=800&color=6A5ACD&center=true&vCenter=true&width=600&lines=Hi+there+%F0%9F%91%8B%2C+I'm+Aditi+Dilip+Kuhar;AI+%26+Data+Science+Undergraduate;AI+%7C+Full-Stack+Developer;LLM+%26+Agent+Systems+Enthusiast" alt="Typing SVG" />
</h1>

🎓 **Artificial Intelligence & Data Science Undergraduate** | 💻 **AI & Full-Stack Developer** | 🤖 **LLM & Agent Systems Enthusiast**

I’m passionate about building **AI-powered applications**, **autonomous agent systems**, and **scalable full-stack solutions**. I enjoy working at the intersection of **machine learning, large language models, and modern web technologies** to solve real-world problems.

---

## 🚀 What I’m Currently Working On

* 🤖 LLM-based **AI Agents & Automation Systems**
* 🧠 **Machine Learning & Data Analysis** projects
* 🌐 **Full-Stack Development** with React & FastAPI
* ⚙️ Designing scalable **APIs & backend architectures**

---

## 🛠️ Tech Stack

**Languages**
Python • JavaScript • Java • SQL

**Frameworks & Libraries**
FastAPI • Flask • React.js • Node.js • Pandas • NumPy • Scikit-learn • TensorFlow

**AI / ML**
LLM Integration (OpenAI, Gemini) • NLP • Predictive Modeling • Feature Engineering

**Databases & Tools**
PostgreSQL • MySQL • MongoDB • Git • GitHub • Docker • VS Code • Postman

---

## 📌 Featured Projects

### 🔹 Codecraft AI

Autonomous multi-agent AI system where Architect, Coder, Tester, and Reviewer agents collaborate to generate production-ready code.

* Python • FastAPI • LangChain • AutoGen
* CI/CD with GitHub Actions

🔗 [View Project](https://github.com/Aditi305/Codecraft-AI)

---

### 🔹 AIAgentFlow

Visual workflow builder to connect AI agents into autonomous pipelines.

* React • FastAPI • OpenRouter API • ReactFlow

🔗 [View Project](https://github.com/Aditi305/AIAgentFlow)

---

## 📊 GitHub Stats

![Aditi's GitHub stats](https://github-readme-stats.vercel.app/api?username=Aditi305\&show_icons=true\&theme=radical)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Aditi305\&layout=compact\&theme=radical)

---

## 📫 Connect With Me

* 📧 Email: [aditikuhar30@gmail.com](mailto:aditikuhar30@gmail.com)
* 💼 LinkedIn: [https://www.linkedin.com/in/aditi-kuhar-540417221](https://www.linkedin.com/in/aditi-kuhar-540417221)
* 🌐 Portfolio: [https://aditi-kuhar-portfolio.vercel.app/](https://aditi-kuhar-portfolio.vercel.app/)
* 💬 Discord: [https://discord.gg/YOUR_INVITE_CODE](https://discord.gg/YOUR_INVITE_CODE)

---

⭐ *Feel free to explore my repositories and connect with me. I’m always open to learning, collaboration, and new opportunities!*
